...
class MainPage(QWidget):
    ...
    def set_text(self, region, t1, t2, t3, t4, t5):
        """지역 정보 텍스트 변경 이벤트 함수"""
        self.comboBox.clear()
        self.title_lbl.setText(f"{region}")
    ...
    def region_info(self, idx):
        ...
        if idx == 0:
            ...
            self.set_text("경   기", "스타필드하남", "스타필드고양", "현대백화점판교점", "애버랜드", "현대프리미엄아울렛김포점")
        elif idx == 2:
            self.set_text("인   천", "현대프리미엄아울렛송도점", "롯데백화점인천점", "소래포구종합어시장", "인천종합어시장", "소래포구")
        elif idx == 3:
            self.set_text("세   종", "세종호수공원", "CGV세종", "세종전통시장", "금강보행교", "메가박스세종나성")
        elif idx == 4:
            self.set_text("대   전", "갤러리아백화점타임월드점", "롯데백화점대전점", "현대프리미엄아울렛대전점", "오정농수산물도매시장",
                          "대전월드컵경기장")
        elif idx == 5:
            self.set_text("대   구", "신세계백화점대구점", "서문시장", "현대백화점대구점", "수성못", "EXCO서관")
        elif idx == 6:
            self.set_text("부   산", "신세계백화점센텀시티점", "롯데백화점부산본점", "롯데프리미엄아울렛동부산점",
                          "해운대해수욕장", "광안리해수욕장")
        elif idx == 7:
            self.set_text("울   산", "현대백화점울산점", "롯데백화점울산점", "진하해수욕장", "일산해수욕장", "태화강국가정원")
        elif idx == 8:
            self.set_text("광   주", "신세계백화점광주점", "롯데백화점광주점", "김대중컨벤션센터", "메가박스광주하남",
                          "롯데아울렛광주수완점")
        elif idx == 9:
            self.set_text("강   원", "속초관광수산시장", "속초해수욕장", "안목해변", "강릉중앙시장", "주문진항")
        elif idx == 10:
            self.set_text("경   남", "롯데백화점창원점", "창원컨벤션센터", "독일마을", "통도사", "진주중앙시장")
        elif idx == 11:
            self.set_text("경   북", "죽도시장", "첨성대", "영일대해수욕장", "롯데백화점포항점", "보문관광단지")
        elif idx == 12:
            self.set_text("전   남", "이순신광장", "죽녹원", "오동도", "향일암", "여수해상케이블카놀아정류장")
        elif idx == 13:
            self.set_text("전   북", "전주한옥마을", "롯데몰군산점", "전주월드컵경기장", "롯데백화점전주점", "전주동물원")
        elif idx == 14:
            self.set_text("충   남", "삽교호관광지", "신세계백화점천안아산점", "독립기념관", "예당호출렁다리", "갤러리아백화점센터시티점")
        elif idx == 15:
            self.set_text("충   북", "현대백화점충청점", "오창호수공원", "단양구경시장", "청풍호반케이블카", "롯데아울렛청주점")
        elif idx == 16:
            self.set_text("제   주", "동문재래시장", "서귀포매일올레시장", "성산일출봉", "함덕해수욕장", "곽지해수욕장")
