    ...
    def receive_message(self):
        ...
        while True:
            ...
            if response_header == self.tourist_name:
                self.tourist_name_str = response_data

            # 부동산 정보
            if response_header == self.realty_info:
                self.realty_data_str = response_data

            # 데이터 시각화를 위한 부동산 정보
            if response_header == self.realty_data:
                self.realty_data = response_data

            # 년도 데이터 정보
            if response_header == self.year_data:
                self.year_data_str = response_data

            # 인프라 데이터 정보
            if response_header == self.infra_data:
                self.infra_data_str = response_data
